...
onMounted(async () => {
  try {
    ...
    message.error("Failed to load finance settings");
  }
});
  ...
  <div class="finance-settings">
    <NCard :title="t('settings.finance.title')" :bordered="false">
      <NForm ref="formRef" label-placement="top">
        <NFormItem label="Default Account">
          <NSelect
        ...
        <NFormItem label="Auto Categorize Transactions">
          <NSwitch
        ...
        <NFormItem label="Auto Reconcile Accounts">
          <NSwitch
            v-model:value="form.autoReconcile"
            :disabled="saving"
          />
          <div class="hint">
            Automatically reconcile accounts with bank statements
          </div>
        </NFormItem>
